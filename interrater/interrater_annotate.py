...
class BaseOptions():
    ...
    def initialize(self):
        ...
        self.parser.add_argument('--stage', type=int, default=None, help='stage to annotate.')
...
# assume we have filtered it before
def getMRList (MRPath):
    mrList = []
    # jedes verzeichnis ist ein kandidat
    candidates = glob(os.path.join(MRPath, "*/"))
    print ("Found", len(candidates), "candidates")
    for cp in candidates:
        if "annotat" in cp or "pred" in cp:
            continue
        sList = {}
        print ( glob(os.path.join(cp, "*/")))
        for sp in glob(os.path.join(cp, "*/")):
            sList[sp] = 1

        bestSeries = min(sList, key=sList.get)
        if sList[bestSeries] == 999:
            continue
        mrList.append(bestSeries)
    return mrList


if  __name__ == '__main__':
    opt = BaseOptions().parse()
    if opt.rater is None or opt.stage is None:
        raise Exception ("Need to specify both, rater and stage!")

    assert (opt.stage > 0)
    assert (opt.stage < 3)

    # check if the cache exists
    if os.path.exists("./IR_cache.json") == False:
        MRPath = "../data/train/PICAI"
        print ("Searching for test MR data in", os.path.join(MRPath, "*/"))
        # mrList = getMRList (MRPath)
        ...
        mrList = mrList[0:5]
        with open("./IR_cache.json", 'w') as file:
            json.dump(mrList, file)

    with open('./IR_cache.json', 'r') as file:
        mrList = json.load(file)

    annotationPath = f"./rater_{opt.rater}_{opt.stage}"
    ...
    while True:
        ...
        try:
            ...
            print(row)
            imgStack, imgAnnotation, (pixelSpacingX, pixelSpacingY, pixelSpacingZ) = loadMRandAnnotation (row)
        except Exception as e:
            print(e)
        ...
        if opt.stage == 2:
            # fix frame to the GT frame
            tmp_csvPath = '_'.join(str(mrList[curMR]).split("/")[-3:-1]) + ".csv"
            tmp_csvPath = os.path.join("../annotations/train/", tmp_csvPath)
            print ("Reading ", tmp_csvPath)
            a = pd.read_csv(tmp_csvPath)
            a = a.query("curFrame > 0").iloc[1:2].copy() # take the middle annotation for PICAI
            assert (len(a) == 1)
            curFrame = a.iloc[0]["curFrame"]
        else:
